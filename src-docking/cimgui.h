...
CIMGUI_API void igTextUnformattedEx(const char* text, const char* text_end /* = NULL */);          // raw text without formatting. Practically equivalent to 'Text("%s", text)' but doesn't require null terminated string if 'text_end' is specified.
...
CIMGUI_API bool  igTreeNodeStr(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);                                         // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
...
CIMGUI_API bool  igTreeNodeGetOpen(ImGuiID storage_id);                                                                         // retrieve tree node open/close state.
...
CIMGUI_API bool igOpenPopup(const char* str_id, ImGuiPopupFlags popup_flags /* = 0 */);                              // call to mark popup as open (don't call every frame!).
CIMGUI_API bool igOpenPopupID(ImGuiID id, ImGuiPopupFlags popup_flags /* = 0 */);                                    // id overload to facilitate calling from nested stacks
CIMGUI_API bool igOpenPopupOnItemClick(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 0 */);      // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
...
CIMGUI_API bool igBeginPopupContextItem(void);                                                                            // Implied str_id = NULL, popup_flags = 0
CIMGUI_API bool igBeginPopupContextItemEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 0 */);        // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
CIMGUI_API bool igBeginPopupContextWindow(void);                                                                          // Implied str_id = NULL, popup_flags = 0
CIMGUI_API bool igBeginPopupContextWindowEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 0 */);      // open+begin popup when clicked on current window.
CIMGUI_API bool igBeginPopupContextVoid(void);                                                                            // Implied str_id = NULL, popup_flags = 0
CIMGUI_API bool igBeginPopupContextVoidEx(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 0 */);        // open+begin popup when clicked in void (where there are no windows).
...
CIMGUI_API void igTableNextRowEx(ImGuiTableRowFlags row_flags /* = 0 */, float min_row_height /* = 0.0f */);      // append into the first cell of a new row. 'min_row_height' include the minimum top and bottom padding aka CellPadding.y * 2.0f.
...
CIMGUI_API void igTableSetupColumn(const char* label, ImGuiTableColumnFlags flags /* = 0 */);      // Implied init_width_or_weight = 0.0f, user_data = 0
CIMGUI_API void igTableSetupColumnEx(const char* label, ImGuiTableColumnFlags flags /* = 0 */, float init_width_or_weight /* = 0.0f */, ImGuiID user_data /* = 0 */);
...
CIMGUI_API ImGuiID igGetWindowDockID(void);                                               // get dock id of current window, or 0 if not associated to any docking node.
...
// Overlapping mode
CIMGUI_API void igSetNextItemAllowOverlap(void);      // allow next item to be overlapped by a subsequent item. Typically useful with InvisibleButton(), Selectable(), TreeNode() covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
...
CIMGUI_API ImGuiItemFlags igGetItemFlags(void);                                            // get generic flags of last item
CIMGUI_API int            igGetItemClickedCountWithSingleClickDelay(void);                 // Implied mouse_button = 0, delay = -1.0f
CIMGUI_API int            igGetItemClickedCountWithSingleClickDelayEx(ImGuiMouseButton mouse_button /* = 0 */, float delay /* = -1.0f */); // [BETA] building block for disambiguation between single-click and double-click. Returns 1 on single-click but delayed by io.MouseSingleClickDelay after mouse release. Returns 2+ on double-click or repeated clicks.
...
CIMGUI_API bool        igIsKeyPressedEx(ImGuiKey key, bool repeat /* = true */);                 // was key pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.
...
CIMGUI_API bool igSetItemKeyOwner(ImGuiKey key);      // Set key owner to last item ID if it is hovered or active. Return true when ownership has been set. Roughly equivalent to 'if (TestKeyOwner(key, GetItemID()) && (IsItemHovered() || IsItemActive())) { SetKeyOwner(key, GetItemID());'.
...
CIMGUI_API bool             igIsMouseReleasedWithDelay(ImGuiMouseButton button);                                             // Implied delay = -1.f
CIMGUI_API bool             igIsMouseReleasedWithDelayEx(ImGuiMouseButton button, float delay /* = -1.f */);                 // delayed mouse release. Use sparingly. Prefer higher-level helper GetItemClickedCountWithSingleClickDelay(). Generally used with 'delay >= io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test.
...
CIMGUI_API ImGuiViewport* igFindViewportByID(ImGuiID viewport_id);                    // this is a helper for backends.
...
CIMGUI_API void   igSetColorEditOptions(ImGuiColorEditFlags flags);      // set current options for if you want to select a default format, picker type, etc. User will be able to change those settings, unless you pass the _NoOptions flag to your calls.
